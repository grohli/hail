...
async def create_vm_config_lambda(
    instance_ip: str,
    activation_token: str,
    machine_name: str,
    instance_config_dict: dict,
    max_idle_time_msecs: int,
    file_store: FileStore,
) -> None:
    """Configure a Lambda VM to run as a Hail batch worker"""

    log.info(f'Configuring Lambda VM at {instance_ip} as batch worker {machine_name}')

    try:
        # Step 1: Transfer credentials to Lambda VM
        await _transfer_credentials(instance_ip)

        # Step 2: Setup Docker authentication and worker
        await _setup_batch_worker(
            instance_ip=instance_ip,
            activation_token=activation_token,
            machine_name=machine_name,
            instance_config_dict=instance_config_dict,
            max_idle_time_msecs=max_idle_time_msecs,
            file_store=file_store,
        )

        log.info(f'Successfully configured Lambda VM {machine_name} at {instance_ip}')

    except Exception as e:
        log.error(f'Failed to configure Lambda VM {machine_name} at {instance_ip}: {e}')
        raise


async def _transfer_credentials(instance_ip: str) -> None:
    """Transfer GCP service account credentials to Lambda VM"""
    log.info(f'Transferring credentials to Lambda VM at {instance_ip}')

    await check_shell(
        f"""scp -i /lambda-ssh-key/lambda-ssh-key.pem -o StrictHostKeyChecking=no /lambda-gsa-key/key.json ubuntu@{instance_ip}:key.json"""
    )


async def _setup_batch_worker(
    instance_ip: str,
    activation_token: str,
    machine_name: str,
    instance_config_dict: dict,
    ...
    """Setup Docker authentication and start the batch worker container"""

    # Encode instance config for passing to Docker container
    instance_config_b64 = base64.b64encode(json.dumps(instance_config_dict).encode()).decode()
...
sudo docker pull {BATCH_WORKER_IMAGE}
sudo mkdir -p /host
sudo mkdir -p /batch
sudo mkdir -p /logs
sudo mkdir -p /global-config
sudo mkdir -p /deploy-config
sudo mkdir -p /cloudfuse
sudo mkdir -p /etc/netns

INSTANCE_CONFIG='{instance_config_b64}'
sudo docker run \\
--name worker \\
-e CLOUD=lambda \\
-e CORES=10 \\
...
-e IP_ADDRESS={instance_ip} \\
-e BATCH_LOGS_STORAGE_URI={file_store.batch_logs_storage_uri} \\
-e INSTANCE_ID={file_store.instance_id} \\
-e PROJECT=hail-vdc \\
-e ZONE=us-central1a \\
-e REGION=us-central \\
...
-e BATCH_WORKER_IMAGE_ID=lambda-worker-image-id \\
...
"""
    ...
    log.info(f'Executing batch worker setup script on Lambda VM at {instance_ip}')

    # Execute the setup script on the Lambda VM
    await check_shell(
        f"""ssh -i /lambda-ssh-key/lambda-ssh-key.pem -o StrictHostKeyChecking=no ubuntu@{instance_ip} 'bash -s > log.txt' <<EOF
{command}
EOF""")
